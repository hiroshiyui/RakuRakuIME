...
- **Static priors — corpus weights.** Each `dictionary` row carries `character_weight` (single-char rows) and `phrase_weight` (multi-char rows), seeded by `CinParser` from `FrequencyCsv.loadAll(context)` which reads `assets/85rest01.csv` (字頻) and `assets/85rest02.csv` (詞頻) — MOE 八十五年常用語詞調查 corpus shipped (UTF-8 transcoded from the government-released Big5 via `iconv -c`) for F-Droid auditability. Weight = `floor(10 000 / rank)` — a reciprocal shape so the long tail still gets differentiated values (rank 1 ⇒ 10 000, rank 1 000 ⇒ 10, past rank 10 000 ⇒ 0). The earlier `max(0, 100 - rank)` formula made the long tail flat, which dropped common but mid-rank phrases (e.g. 輕鬆 at rank 1 424) below alphabetic order in the next-char prediction strip. `DictionaryDao` orders candidates by `frequency DESC, (character_weight + phrase_weight) DESC` — user learning still wins long-term, but day-1 ordering of fresh installs reflects the corpus prior. `nextCharactersAfter` adds `SUM(phrase_weight) DESC` as its tiebreaker for the same reason.
- **Next-character prediction.** `DictionaryDao.nextCharactersAfter(likePattern, prefixLen, limit)` projects the 2nd char of every multi-char `dictionary` entry whose `character` matches `likePattern` (e.g. `信%`), grouped and ordered by `SUM(frequency) DESC, SUM(phrase_weight) DESC`. Drives the post-selection strip described above. The LIKE scan is unindexed — acceptable because it only fires on candidate commit (not per keystroke) over a ~130k-row corpus. No new asset; the prediction signal is mined from the same phrases already shipped in `ezbig.utf-8.cin`.
- **User phrases (User Phrase Manager).** A separate Room table `user_phrases` (entity `UserPhraseEntry`, DAO `UserPhraseDao`, added in v4 via `MIGRATION_3_4`, gained per-row `frequency` in v5 via `MIGRATION_4_5`) holds user-defined phrases. Each row is `(character, keystroke, created_at, frequency)` with a unique index on `(character, keystroke)` — duplicate inserts collapse via `OR IGNORE`. The Manager screen offers inline edit through `UserPhraseDao.updateById` (`UPDATE OR IGNORE` so a collision with the unique index becomes a 0-row return that the UI surfaces as a friendly toast) — edit-in-place preserves `id`, `created_at`, and learned `frequency` instead of forcing a delete-and-recreate cycle. **Learned-frequency policy:** `KeyboardViewModel.selectCandidate` bumps the frequency on *both* DAOs unconditionally — the UPDATE on whichever table doesn't own the committed row is a no-op, so the ViewModel doesn't need to know which side the candidate came from. User-phrase candidate queries sort by `frequency DESC, created_at DESC`; the always-rank-first merge in the ViewModel still wins over corpus rows so frequency only orders user phrases among themselves. The asset DB stays at v3 (corpus only); the table is created empty on first launch after upgrade. **Ranking policy:** user phrases are *always* prepended to corpus candidates for the same keystroke (and the same is true of next-char predictions); `KeyboardViewModel.updateCandidates` / `updateNextCharPredictions` queries both DAOs and uses `mergeUnique` to dedupe. This deliberately overrides corpus frequency and `frequency`-learned ordering so the user's explicit additions are predictable. **Auto-select interaction:** the "only one possible completion ⇒ auto-commit" optimisation runs against the *corpus* signal only (`corpusPrefix.size == 1 && userPrefix.isEmpty()`). Counting user phrases here is unsafe because the corpus FTS prefix query silently returns empty for keystrokes containing chars the FTS4 tokenizer doesn't index (commas, backticks…), so a lone user-phrase hit would falsely look like "only one possible completion" and auto-commit. Backup/restore is CSV (`"character","keystroke"` header, RFC-4180 quoting) handled by the JVM-testable `UserPhraseCsv`; restore validates the EZ root charset using `CinParser.validKeystrokeChars(context)`, which lazily parses the `%keyname` block of the bundled CIN. The Compose `UserPhraseManagerScreen` is reached via a sealed-class screen-state switch in `MainActivity`, not a NavHost.
- **Keystroke suggestion (`EzKeystrokeLookup`).** The Add-Phrase dialog's "Analyse Roots" button suggests EZ root strings for a user-entered phrase. **Implements the EZ 取碼規則** described in <https://github.com/hiroshiyui/EzIM_Tables_Project/blob/main/CLAUDE.md>, not a naïve char-by-char concatenation. Per-phrase length rule: 1 char → the chosen full encoding; 2 chars → each char's full encoding concatenated; 3 chars → each char's 頭 (first key of chosen encoding); 4 chars → same, four heads; 5+ chars → only the first four chars' heads (later chars ignored). Per-character encoding selection: single-key encoding wins, else non-all-digit wins, else first in CIN-file order (rows are inserted in file order so `DictionaryDao.encodingsForCharacter` uses `ORDER BY id ASC`). Phrase-level keystrokes already present in the corpus are surfaced *first* via `DictionaryDao.keystrokesForPhrase` (which, unlike `encodingsForCharacter`, doesn't filter to single-char rows) — if the EZ tables already encode this exact phrase, that canonical answer leads the list. The per-character rule then contributes the strict char-by-char canonical pick, followed by alternates generated by trying other valid encodings at each position. If a character is missing from the corpus, the char-by-char fallback bails but any phrase-level hits already collected still stand; with no hits at all, the result is empty and the user falls back to manual entry. The picking-rule helper (`sortByEzPickingRule`) is `internal` so a JVM unit test can exercise it without a DB.
- **Backup / Restore (`BackupArchive`).** Settings → "Backup & Restore" exports user phrases + learned dictionary frequencies as a gzipped JSON archive (`*.rkbak.gz`), and restores them by merging into the existing DB. Format header: `{schema: 1, applicationId, createdAt}`; payload: `userPhrases[]` (full rows incl. `frequency` + `createdAt`) and `dictionaryFrequencies[]` (only rows with `frequency > 0` — the static corpus is shipped in the asset DB, so backing it up would just bloat the archive). The parser treats the file as untrusted: 50 MiB uncompressed cap to defeat gzip bombs, strict-decode rejection of unknown top-level *and* nested fields, per-row string length caps, `isISOControl` rejection in phrase text, and EZ keystroke validation via the same `CinParser.validKeystrokeChars` validator the CRUD UI uses. The first validation failure aborts the whole import — never apply a partial restore. Restore-side: user phrases via `OR IGNORE` against the unique `(character, keystroke)` index; dictionary frequencies via `DictionaryDao.incrementFrequencyExactBy` (delta-add) so a partial restore doesn't clobber recent learning. JVM-testable via `BackupArchive.parse(InputStream, Set<Char>)` — host tests need `org.json:json` on the test classpath since Android stubs out `org.json` on JVM hosts.
...
- `databases/ime_database.db` — pre-packaged Room DB built from `ezbig.utf-8.cin` + `85rest01.csv` + `85rest02.csv`. Rebuild with `./gradlew :app:buildImeDb` after touching any of those inputs, the Room schema, or `FrequencyCsv.SEED_NUMERATOR`. The Gradle task is registered in `app/build.gradle.kts`; the heavy lifting lives in `buildSrc/src/main/kotlin/ImeDbBuilder.kt`. The task reproduces Room's exact v3 schema (table, FTS4 virtual table, sync triggers, `room_master_table` identity hash read from `app/schemas/.../3.json`) so the asset is byte-compatible with what Room would generate at runtime.
